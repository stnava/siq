...
if len(sys.argv) <= 1:
    print("usage:")
    print(sys.argv[0] +
          " n  paramfile.csv rootdir  cudanum index output_prefix ")
...
myoutprefix = './models/siq_smallshort_train_'+uppername+'_' + str(nchan) + feetext
if outimageflename == "":
...
maxit2 = 800
if not doseg:
    # should match ratio of high to low size patches
    mdl = siq.default_dbpn(upper, nChannelsIn=nchan, nChannelsOut=nchan,
    ...
    with options({'constant_folding': True}):
        training_path = siq.train(
            ...
            feature=2.0,
            tv=0.1,
            max_iterations=maxit2+1,
            verbose=True)
else:
    mdl = siq.default_dbpn(upper,
                           nChannelsIn=nchan, nChannelsOut=nchan,
                           sigmoid_second_channel=True, option='small')  # should match ratio of high to low size patches
